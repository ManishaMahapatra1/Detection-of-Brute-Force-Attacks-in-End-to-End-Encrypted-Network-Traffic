#!/usr/bin/env zeek
#dest-oriented_short_connection_https.zeek
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="HTTPSDestOrientedShortConnectionsBruteForce short connection duration observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting HTTPSDestOrientedShortConnectionsBruteForce short connection durations", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["HTTPSDestOrientedShortConnectionsBruteForce short connection duration observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("HTTPSDestOrientedShortConnectionsBruteForce short connection duration observed",
