#!/usr/bin/env zeek
#orig-oriented_equal_sized_https.zeek

module HTTPSEqualSizedResponsesBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="HTTPSEqualSizedResponsesBruteForce response size observed", $apply=set(SumStats::TOPK));

  SumStats::create([
    $name = "counting HTTPSEqualSizedResponsesBruteForce response sizes", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local topk = result["HTTPSEqualSizedResponsesBruteForce response size observed"]$topk;
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("HTTPSEqualSizedResponsesBruteForce response size observed",
