#!/usr/bin/env zeek
#orig-oriented_similar_sized_https.zeek

module HTTPSDestOrientedSimilarSizedResponsesBruteForce;
...
event zeek_init() &priority=15 {
  local reducer = SumStats::Reducer($stream="HTTPSDestOrientedSimilarSizedResponsesBruteForce response size observed", $apply=set(SumStats::TOPK));

  SumStats::create([
    $name = "counting HTTPSDestOrientedSimilarSizedResponsesBruteForce response sizes", $epoch=epoch_len, $reducers=set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local topk = result["HTTPSDestOrientedSimilarSizedResponsesBruteForce response size observed"]$topk;
      ...
      for (tcp_size in size_counts) {
        ...
        for (comp_size in size_counts) {
          if (comp_size < tcp_size) {
              local temp = comp_size;
              comp_size = tcp_size;
              tcp_size = temp;
          }
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("HTTPSDestOrientedSimilarSizedResponsesBruteForce response size observed",
